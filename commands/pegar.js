...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if (!args[0]) return message.reply(`<a:recusar:1072582282315038802> | Você não selecionou nenhum ID de compra!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      if(args[0] !== `${db3.get(`${args[0]}.id`)}`) return message.reply(`<a:recusar:1072582282315038802> | Esse ID de compra não é existente!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField(`<:estoque:1072582246197903451> | Produto entregue:`, `\`\`\`${db3.get(`${args[0]}.entrid`)}\`\`\``)
